...
# The following mysql is just to veryfy the connection to the DB server.
...
# Set some env variables for the app to use.
...
# Start the app and log the output.
...
resource "azurerm_linux_virtual_machine_scale_set" "web_vmss" {
  ...
  # 1. Create VMSS only afterJava App related DB Schema "webappdb" is created in MySQL Server
